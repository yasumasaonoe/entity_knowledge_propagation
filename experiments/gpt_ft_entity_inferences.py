...
def main(epoch):
    ...
    ft_model_name = None
    model_info = 'ft_per_ex_ei'

    # Choose a unique experiment name
    exp_name = f'entity_inference/gpt2/final' \
               f'/{ki_method}_{model_info}'
    ...
    train_params = {
        ...
        "EXP_DIR": exp_dir,
        "KI_METHOD": ki_method,
        "BASE_MODEL": "gpt2-xl", #gpt-neo-1.3B",  # model_type: t5-base/t5-large
        "TRAIN_BATCH_SIZE": 1,  # training batch size
        "VALID_BATCH_SIZE": 1,  # validation batch size
        "TRAIN_EPOCHS": epoch,  # number of training epochs
        "VAL_EPOCHS": 1,  # number of validation epochs
        "LEARNING_RATE": 5e-4,  # learning rate 5e-4
        ...
        "COMPUTE_SPECIFICITY": False,  # Set False if prepend_def
        "FREEZE_LAYERS": False,
        "REG_TYPE": '',
        "ALPHA": 0.0
    }
    ...
    all_numbers = {
        'pre_accuracy': total_pre_acc,
        'post_accuracy': total_post_acc,
        'pre_specificity': np.mean(all_specificity_pre_acc),
        'post_specificity': np.mean(all_specificity_post_acc)
    }
